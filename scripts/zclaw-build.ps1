...
function Resolve-EspIdfSetup {
    ...
    throw "ESP-IDF not found. Install ESP-IDF first and make sure tools\\idf_tools.py and a python_env\\...\\Scripts\\python.exe exist under C:\\Espressif or `$env:IDF_PATH."
}
...
if ($FullClean) {
    $commands += (New-IdfCommand -PythonPath $idfSetup.PythonPath -IdfPath $idfSetup.IdfPath -ToolsPath $idfSetup.ToolsPath -Arguments @("fullclean"))
}

$commands += (New-IdfCommand -PythonPath $idfSetup.PythonPath -IdfPath $idfSetup.IdfPath -ToolsPath $idfSetup.ToolsPath -Arguments @("build"))

Write-Host "Building zclaw for $Target"
