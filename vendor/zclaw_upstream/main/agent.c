...
static SemaphoreHandle_t s_agent_mutex;
...
static char *s_captured_response_buf = NULL;
static size_t s_captured_response_buf_size = 0;
static bool s_captured_response_written = false;
static agent_status_cb_t s_status_cb = NULL;
static void *s_status_ctx = NULL;
static llm_stream_chunk_cb_t s_stream_chunk_cb = NULL;
static void *s_stream_chunk_ctx = NULL;
...
{
    if (s_captured_response_buf && s_captured_response_buf_size > 0) {
        strncpy(s_captured_response_buf, text, s_captured_response_buf_size - 1);
        s_captured_response_buf[s_captured_response_buf_size - 1] = '\0';
        s_captured_response_written = true;
        return;
    }
...
static void agent_emit_status(const char *event, const char *message)
{
    if (!s_status_cb || !event || event[0] == '\0') {
        return;
    }
    s_status_cb(event, message, s_status_ctx);
}
...
static void handle_dht_command(int64_t chat_id, request_metrics_t *metrics)
{
    bool ok;
    int64_t started_us;
    ...
    s_tool_result_buf[0] = '\0';
    started_us = esp_timer_get_time();
    ok = local_admin_read_dht11(s_tool_result_buf, sizeof(s_tool_result_buf));
    metrics->tool_us_total += elapsed_us_since(started_us);
    metrics->tool_calls++;

    if (!ok) {
        if (s_tool_result_buf[0] == '\0') {
            snprintf(s_tool_result_buf, sizeof(s_tool_result_buf), "Error: DHT11 read failed");
        }
        send_response(s_tool_result_buf, chat_id);
        metrics_log_request(metrics, "dht_failed");
        return;
    }

    send_response(s_tool_result_buf, chat_id);
    metrics_log_request(metrics, "dht_handled");
}

static bool is_local_admin_command(const char *user_message)
{
    return agent_is_command(user_message, "gpio") ||
           agent_is_command(user_message, "dht") ||
                                       ...
                                       request_metrics_t *metrics)
{
    ...
    if (agent_is_command(user_message, "dht")) {
        handle_dht_command(chat_id, metrics);
        return;
    }
...
static void handle_settings_command(int64_t chat_id)
{
    ...
    snprintf(settings_text, sizeof(settings_text),
             ...
             "- USB local admin: /gpio, /dht, /diag, /reboot, /wifi, /bootcount, /factory-reset\n"
             "- /gpio supports reads and writes (e.g. /gpio 9 low)\n"
             "- /dht reads the DHT11 sensor on GPIO5\n"
...
static void process_message(const char *user_message, message_source_t source, int64_t reply_chat_id)
{
    ...
    if (!s_runtime_ready) {
        send_response("System is still starting up. Try again in a few seconds, or use /wifi status and /diag quick.", reply_chat_id);
        metrics_log_request(&metrics, "startup_not_ready");
        return;
    }
    ...
    while (!done && rounds < MAX_TOOL_ROUNDS) {
        ...
        for (int retry = 0; retry < LLM_MAX_RETRIES; retry++) {
            ...
            err = llm_request_streaming(request,
                                        s_response_buf,
        ...
        if (tool_name[0] != '\0' && tool_input) {
            ...
            agent_emit_status("tool_call", tool_name);
            ...
            agent_emit_status("tool_result", s_tool_result_buf);
                      ...
                      QueueHandle_t telegram_output_queue)
{
    ...
    s_telegram_output_queue = telegram_output_queue;
    if (!s_agent_mutex) {
        s_agent_mutex = xSemaphoreCreateMutex();
        if (!s_agent_mutex) {
            ESP_LOGE(TAG, "Failed to create agent mutex");
...
{
    return agent_process_web_request_streaming(user_message,
                                               response_buf,
                                               response_buf_size,
                                               NULL,
                                               NULL,
...
esp_err_t agent_process_web_request_streaming(const char *user_message,
                                             ...
                                             void *stream_ctx)
{
    if (!user_message || !response_buf || response_buf_size == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    if (!s_agent_mutex) {
    ...
    response_buf[0] = '\0';
    xSemaphoreTake(s_agent_mutex, portMAX_DELAY);
    s_captured_response_buf = response_buf;
    s_captured_response_buf_size = response_buf_size;
    s_captured_response_written = false;
    s_status_cb = status_cb;
    s_status_ctx = status_ctx;
    s_stream_chunk_cb = stream_cb;
    s_stream_chunk_ctx = stream_ctx;
    process_message(user_message, MSG_SOURCE_WEB, 0);
    s_captured_response_buf = NULL;
    s_captured_response_buf_size = 0;
    s_status_cb = NULL;
    s_status_ctx = NULL;
    s_stream_chunk_cb = NULL;
    s_stream_chunk_ctx = NULL;
    xSemaphoreGive(s_agent_mutex);

    return s_captured_response_written ? ESP_OK : ESP_FAIL;
}
