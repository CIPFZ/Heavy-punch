...
static esp_err_t ensure_wifi_stack_ready(void)
{
    ...
    if (sta_netif) {
        ...
        device_identity_get_hostname(hostname, sizeof(hostname));
        if (hostname[0] != '\0') {
            err = esp_netif_set_hostname(sta_netif, hostname);
            if (err != ESP_OK) {
                ESP_LOGW(TAG, "Failed to set STA hostname: %s", esp_err_to_name(err));
            }
        }
    }
...
{
    return agent_is_command(message, "dht") ||
           agent_is_command(message, "reboot") ||
                                ...
                                local_admin_action_t *action_out)
{
    ...
    if (agent_is_command(message, "dht")) {
        payload = agent_command_payload(message, "dht");
        if (payload && payload[0] != '\0') {
            snprintf(result, result_len, "Error: /dht does not take arguments");
            return false;
        }
    ...
    if (agent_is_command(message, "bootcount")) {
        ...
        if (payload && payload[0] != '\0') {
            ...
            extra = strtok(NULL, " \t\r\n");
            if (!token || extra != NULL) {
                snprintf(result, result_len, "Error: use /bootcount or /bootcount reset");
                return false;
            }

            if (strcmp(token, "reset") != 0 && strcmp(token, "clear") != 0) {
                snprintf(result, result_len, "Error: use /bootcount or /bootcount reset");
                return false;
            }

            if (action_out) {
                *action_out = LOCAL_ADMIN_ACTION_RESET_BOOTCOUNT_REBOOT;
            }
            snprintf(result, result_len, "Boot count cleared. Rebooting...");
            return true;
        }
...
void local_admin_perform_action(local_admin_action_t action)
{
    ...
    if (action == LOCAL_ADMIN_ACTION_RESET_BOOTCOUNT_REBOOT) {
        (void)boot_guard_set_persisted_count(0);
    } else if (action == LOCAL_ADMIN_ACTION_FACTORY_RESET_REBOOT) {
    ...
    if (action == LOCAL_ADMIN_ACTION_RESET_BOOTCOUNT_REBOOT) {
        esp_err_t err = boot_guard_set_persisted_count(0);
        if (err != ESP_OK) {
            channel_write("\r\nFailed to clear boot count.\r\n\r\n");
            return;
        }
    } else if (action == LOCAL_ADMIN_ACTION_FACTORY_RESET_REBOOT) {
...
bool local_admin_read_dht11(char *result, size_t result_len)
{
    return dht11_read(result, result_len);
}
